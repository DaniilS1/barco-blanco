...
export default function ProductDetails({ productData, similarProducts = [], isLoading = false }: ProductDetailsProps) {
  ...
  const MOBILE_CONTAINER_WIDTH = 360;
  ...
  const goToPrevious = () => {
    ...
    else if (images.length > 1) setSelectedImageIndex(images.length - 1);
  };

  const goToNext = () => {
    if (selectedImageIndex < images.length - 1) setSelectedImageIndex(selectedImageIndex + 1);
    else if (images.length > 1) setSelectedImageIndex(0);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-4 md:py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-12">
          ...
          <div className="relative flex items-center justify-center">
            {images.length > 1 && (
              <button
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-[#1996A3] hover:bg-white text-white hover:text-[#1996A3] transition-all duration-200 w-12 h-12 rounded-full shadow-sm flex items-center justify-center z-10 border border-[#1996A3] hidden md:flex"
            ...
            {images.length > 1 && (
              <button
                ref={nextRef}
                onClick={goToNext}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-[#1996A3] hover:bg-white text-white hover:text-[#1996A3] transition-all duration-200 w-12 h-12 rounded-full shadow-sm flex items-center justify-center z-10 border border-[#1996A3] hidden md:flex"
        ...
        <div className="w-full lg:w-1/2 flex flex-col gap-5 md:gap-7 order-2 lg:order-2">
          {/* Product Title */}
          <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 leading-tight">{name}</h1>
          ...
          <div className="flex items-center justify-between md:justify-start gap-4 flex-wrap">
            ...
            <div className="flex items-center border border-gray-300 rounded-lg sm:px-2 sm:py-1 sm:hover:text-[#1996A3] sm:transition-colors sm:rounded-l-lg">
              <button 
              ...
              <span className="px-4 py-2 border-x border-gray-300 min-w-[3rem] text-center font-medium">{quantity}</span>
              <button 
          ...
          <div className="flex items-center gap-3 text-sm text-gray-600">
            <div className="flex items-center gap-2 hover:text-[#1996A3] transition-colors cursor-pointer">
